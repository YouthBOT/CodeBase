                ...
                if (reader.HasRows) {
                    ...
                    if ((match.tournamentId == 5) && (match.matchNumber > 100)) {
                        if (match.matchNumber.ToString().EndsWith("2")) {
                            ...
                            if (reader.HasRows) {
                                await reader.ReadAsync();
                                var firstMatchScore = Convert.ToInt32(reader[0]);
                                reader.Close();
                                ...
                                query = "UPDATE matches " +
                                    "SET played = 1, " +
                                    string.Format("green_team = {0}, green_score = {1}, green_result = '{2}' ",
                                        match.greenTeam, averageScore, "A") +
                                ...
                                if ((roundNumber % 2 == 0) && (roundNumber < 30)) {
                                    ...
                                    Console.WriteLine("Reading matches {0} thru {1}", lowerBound, upperBound);
                                    ...
                                    while (await reader.ReadAsync()) {
                                        if (reader[13].ToString() == "S") {
                                            ...
                                            Console.WriteLine("Team: {0}, Score: {1}", match.greenTeam, match.greenScore);
                                    ...
                                    if (matches.Count == 4) {
                                        ...
                                        query = "INSERT INTO matches " +
                                            ...
                                            string.Format("VALUES ({0}, {1}, 0, {2});", match.tournamentId, match.matchNumber, winningTeam);
                                    }
                                }
                            } else {
                                reader.Close();
